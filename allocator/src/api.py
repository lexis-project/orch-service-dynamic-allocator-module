...
class APIRest:
    ...
    def auth(self, token):
        ...
        try:
            data = {'token':token}
            r = requests.post(self.keycloak_URL+'/auth/realms/'+self.KC_REALM+'/protocol/openid-connect/token/introspect', data=data, auth=(self.KC_CLID, self.KC_SECRET), verify=False)
            r.raise_for_status()
    ...
    def exchange_token(self, token):
        ...
        try:
            ...
            r = requests.post(self.keycloak_URL+'/auth/realms/'+self.KC_REALM+'/protocol/openid-connect/token', data=data, verify=False)
            r.raise_for_status()
            if (r.status_code == 200):
                msg = r.json()
    ...
    def refresh_token(self, token):
        ...
        try:
            ...
            r = requests.post(self.keycloak_URL+'/auth/realms/'+self.KC_REALM+'/protocol/openid-connect/token', data=data, auth=(self.KC_CLID, self.KC_SECRET), verify=False)
            r.raise_for_status()
            if (r.status_code == 200):
                msg = r.json()
                if "access_token" in msg.keys() and "refresh_token" in msg.keys():
    ...
    def get_programmed_maintenance(self, cluster):
        ...
        if (validFlag == False):
            ...
            jmsg['status'] = 'err'
            self.logger.doLog("served '/maintenance/dates/%s' [err: cluster %s is not valid]" % (cluster, cluster))
            state = 400
        else:
            # check if the heappe client has been set up or not -- in the latter case, it just use internal
            # in memory database for getting information (actually at the moment return None)
            if (self.platform.heappe == False):
                ...
                if (tmp == None):
                    ...
                    # this is kept for compatibility with thee previous version of the BLU
                    dates_iso = []
                    for item in tmp:
                    ...
                    jmsg['status'] = "ok"
                    state = 200
            # in this case, we are gettting information from the influxDB client 3
            else:
                self.testDbActive()
                if (self.db_active3 == True):
                    # call the external executable script to get the list of entries
                    ret = cli.run(['./get_maintenance.sh', cluster], stdout=False)
                    self.logger.doLog("running external script for querying the database [ok]")
                    ...
                    ret = cli.run(['rm', '../dbs/lxm_maintenance_date.txt'], stdout=False)
                    jmsg = {'message':jptv}
                    jmsg['status'] = 'ok'
                    state = 200
                    self.logger.doLog("served '/maintenance/dates/' (cluster: %s) [ok]" % (cluster))
                else:
                    jmsg = {'message':'the maintenance database is not available -- request failed'}
                    jmsg['status'] = 'err'
                    state = 400
                    self.logger.doLog("served '/maintenance/dates/' (cluster: %s) [err: maintenance database is not available]" % (cluster))
        return(jmsg, state)
    ...
    def get_speed_perf(self, src, dst, size):
        ...
        if ((center_exist_s == True) and (center_exist_d == True)):
            self.testDbActive()
            if (self.db_active1 == True):
                # call the external executable script to get the list of entries
                ...
                state = 200
                if (float(size) <= 0):
                    self.logger.doLog("served '/list/netperf' (src: %s, dst: %s) [ok]" % (src, dst))
                else:
                    self.logger.doLog("served '/load/netperf' (src: %s, dst: %s, size: %s) [ok]" % (src, dst, size))
            else:
                jmsg = {'message':'the netperf is not available -- request failed'}
                jmsg['status'] = 'err'
                state = 400
                if (float(size) <= 0):
                    self.logger.doLog("served '/list/netperf' (src: %s, dst: %s) [err: netperf database is not available]" % (src, dst))
                else:
                    self.logger.doLog("served '/load/netperf' (src: %s, dst: %s, size: %s) [err: netperf database is not available]" % (src, dst, size))
        else:
        ...
        if ret[1] == 200:
            ...
            for item in ret[0]['message']:
                if size != item['size']:
                    ...
                    delete_ret = self.idb_c1.query('DELETE FROM "networkEvaluation" WHERE time = %d' % (timestamp))
            if (found == True):
                jmsg['message'] = "netperf item removed for dest center '%s', source center '%s' and size '%s'" % (str(dst), str(src), str(size))
            else:
                jmsg['message'] = "no previous netperf items for dest center '%s', source center '%s' and size '%s'" % (str(dst), str(src), str(size))
        ...
        if (self.db_active2 == True):
            for queue in job_list['val']:
                ...
                if job_list['params']['type'] == "cloud" or ( job_list['params']['type'] == "both" and "cluster_id" not in queue['dest'].keys() ) :
                    ...
                    m = str(queue['dest']['location']) + "_" + str(queue['dest']['cluster_id'])
                    jmm[m] = queue['mean']
                r += "%s:%.5f" % (m, jmm[m])
                start = False
    ...
    def shutdown_server(self):
        ...
        if (shutdown is None):
            self.logger.doLog("unable to shutdown the server -- not running with 'Werkzeug Server'")
    ...
    def manageRoutes(self):
        ...
        def service_shutdown():
            ...
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/service/shutdown' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
        ...
        def store_netperf(measure):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/store/netperf/<string:measure>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            state = 200
            jmsg = {}
            center_exist_s = True
            center_exist_d = True
            if args['src'] not in self.platform.center_list or args['src'] == args['dst']:
                center_exist_s = False
            ...
            if ((center_exist_s == True) and (center_exist_d == True) and (self.db_active1 == True)):
                temp = self.delete_speed_perf(args['src'], args['dst'], args['size'])
                if temp[1] != 200:
                    self.logger.doLog("error when deleting old entry '/store/netperf/' (%s -> %s) [err]" % (args['src'], args['dst']))
                    return (jsonify(temp[0]), temp[1])
                jmsg['message'] = "performance of the network for '%s -> %s' added" % (args['src'], args['dst'])
                jmsg['status'] = ''
                measure = args['size'] + "_" + measure
                db_body = [{'measurement':'networkEvaluation', \
                            'tags':{'center_src':args['src'], 'center_dst':args['dst'], 'id':self.idx1}, \
                            'fields':{'performance':measure}}]
                self.idx1 += 1
                ret = self.idb_c1.write_points(db_body)
                if (ret == True):
                    jmsg['status'] = 'ok'
                    self.logger.doLog("served '/store/netperf/' (%s -> %s) [ok]" % (args['src'], args['dst']))
                else:
                    jmsg['status'] = 'err'
                    self.logger.doLog("served '/store/netperf/' (%s -> %s) [err: operation failed]" % (args['src'], args['dst']))
                    state = 400
            else:
                # manage the error
                if (self.db_active1 == False):
                    jmsg['message'] = 'target stream database is not acitve or has been deleted'
                    jmsg['status'] = 'war'
                    self.logger.doLog("served '/store/netperf/' (%s -> %s) [war: target stream-db is not active or has been deleted]" % (args['src'], args['dst']))
                    self.logger.doLog("target stream database is not acitve or has been deleted")
                else:
                    jmsg['message'] = "source cluster '%s' and/or destination cluster '%s' do not exist" % (args['src'], args['dst'])
                    jmsg['status'] = 'err'
                    self.logger.doLog("served '/store/netperf/' (%s -> %s) [err: cluster(s) not exist]" % (args['src'], args['dst']))
                    state = 400
            return (jsonify(jmsg), state)
       
 
        # listing all the pairs size_per_file and transfer_speed for a given source and destination locations and for a given size_per_file        
        ...
        def load_netperf():
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/load/netperf' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
        ...
        def delete_netperf():
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/delete/netperf' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            ret = self.delete_speed_perf(args['src'], args['dst'], args['size'])
            return (jsonify(ret[0]), ret[1])


        # listing all the pairs size_per_file and transfer_speed for a given source and destination locations
        ...
        def list_netperf():
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/list/netperf' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
        ...
        def removedb(database):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/removedb/<string:database>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            state = 200
            jmsg = {}
            if (database == self.db_name_1):
                self.idb_c1.drop_database(self.db_name_1)
                jmsg['message'] = "database '%s' has been removed correctly" % (database)
                jmsg['status'] = "ok"
                ...
                jmsg['status'] = "err"
                self.logger.doLog("served '/removedb/%s' [err: database does not exist]" % (database))
                state = 400
            return (jsonify(jmsg), state)
        ...
        def dumpdb(database):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/dumpdb/<string:database>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            state = 200
            jmsg = {}
            if (database == self.db_name_1):
                # influxd backup -portable -database $1 /home/di39mal/devel/flask/lexis_db/databases/_$1_dump
                target = db_dump_path + '/_' + self.db_name_1 + '_dump'
                ret = cli.run(['influxd', 'backup', '-portable', '-database', self.db_name_1, target])
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly dumped" % (database)
                    jmsg['status'] = "ok"
                    self.logger.doLog("served '/dumpdb/%s' [ok: exit code is %d]" % (database, ret.returncode))
                else:
                    jmsg['message'] = "database %s has not correctly dumped" % (database)
                    jmsg['status'] = "err"
                    state = 400
                    self.logger.doLog("served '/dumpdb/%s' [err: exit code is %d]" % (database, ret.returncode))            
            elif (database == self.db_name_2):
                # influxd backup -portable -database $1 /home/di39mal/devel/flask/lexis_db/databases/_$1_dump
                target = db_dump_path + '/_' + self.db_name_2 + '_dump'
                ret = cli.run(['influxd', 'backup', '-portable', '-database', self.db_name_2, target]) 
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly dumped" % (database)
                    jmsg['status'] = "ok" 
                    self.logger.doLog("served '/dumpdb/%s' [ok: exit code is %d]" % (database, ret.returncode))
                ...
                ret = cli.run(['influxd', 'backup', '-portable', '-database', self.db_name_3, target])
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly dumped" % (database)
                    jmsg['status'] = "ok" 
                    self.logger.doLog("served '/dumpdb/%s' [ok: exit code is %d]" % (database, ret.returncode))
        ...
        def restoredb(database):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/restoredb/<string:database>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            state = 200
            jmsg = {}
            # restore procedure: (step 1) drop down the current database; (step 2) restore the database from the file
            if (database == self.db_name_1):
                target = db_dump_path + '/_' + self.db_name_1 + '_dump'
                self.idb_c1.drop_database(self.db_name_1)
                ret = cli.run(['influxd', 'restore', '-db', self.db_name_1, '-portable', target])  
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly restored" % (database)
                    jmsg['status'] = "ok"
                    self.logger.doLog("served '/restoredb/%s' [ok: exit code is %d]" % (database, ret.returncode))
                else:
                    jmsg['message'] = "database %s has not correctly restored" % (database)
                    jmsg['status'] = "err"
                    state = 400
                    self.logger.doLog("served '/restoredb/%s' [err: exit code is %d]" % (database, ret.returncode))
            elif (database == self.db_name_2):
                # influxd backup -portable -database $1 /home/di39mal/devel/flask/lexis_db/databases/_$1_dump
                target = db_dump_path + '/_' + self.db_name_2 + '_dump'
                self.idb_c2.drop_database(self.db_name_2)
                ret = cli.run(['influxd', 'restore', '-db', self.db_name_2, '-portable', target]) 
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly restored" % (database)
                    jmsg['status'] = "ok" 
                    self.logger.doLog("served '/restoredb/%s' [ok: exit code is %d]" % (database, ret.returncode))
                ...
                target = db_dump_path + '/_' + self.db_name_3 + '_dump'
                self.idb_c3.drop_database(self.db_name_3)
                ret = cli.run(['influxd', 'restore', '-db', self.db_name_3, '-portable', target])  
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly restored" % (database)
                    jmsg['status'] = "ok"
                    self.logger.doLog("served '/restoredb/%s' [ok: exit code is %d]" % (database, ret.returncode))
        ...
        def get_programmed_maintenance_api(cluster):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/maintenance/dates/<string:cluster>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
        ...
        def get_maintenance_clusters():
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/maintenance/clusters' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            ...
            if len(self.platform.clusters_list) != 0:
                ...
                self.logger.doLog("served '/maintenance/clusters' no clusters available! [war]")
        ...
        def add_programmed_maintenance(cluster):
            state = 200
            parser = reqparse.RequestParser()
            parser.add_argument('date', type=inputs.iso8601interval, help='Invalid format: use ISO8601 intervals', required=True)
            parser.add_argument('Authorization', type=str, required=True, location='headers')
            args = parser.parse_args()
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/maintenance/<string:cluster>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            dates = args['date']
            jmsg = {}
            try:
                ...
                jmsg['status'] = 'err'
                self.logger.doLog("served '/maintenance/%s' [err: wrong date format]" % (cluster))
                state = 400
                return (jsonify(jmsg), state)
            if start_date > end_date:
                jmsg['message'] = "maintenance start date is after end date!"
                jmsg['status'] = 'err'
                self.logger.doLog("served '/maintenance/%s' [err: start date after end date]" % (cluster))
                state = 400
                return (jsonify(jmsg), state)
            validFlag = True
            ...
            if (validFlag == False):
                ...
                jmsg['status'] = 'err'
                self.logger.doLog("served '/maintenance/%s' [err: cluster does not exist]" % (cluster))
                state = 400
                return (jsonify(jmsg), state)
            else:
                # cluster exist, so try to insert the date in the maintenance database
                self.testDbActive()
                if (self.db_active3 == True):
                    db_body = [{'measurement':'systemMaintenance', \
                            'tags':{'cluster':cluster, 'id':self.idx3}, \
                            'fields':{'start_maintenance':start_date, 'end_maintenance':end_date}}]
                    self.idx3 += 1
                    ret = self.idb_c3.write_points(db_body)
                    if (ret == True):
                        jmsg['status'] = 'ok'
                        self.logger.doLog("served '/maintenance/%s' (start_maintenance: %s / end_maintenance: %s) [ok]" % (cluster, start_date, end_date))
                    else:
                        jmsg['status'] = 'err'
                        self.logger.doLog("served '/maintenance/%s' (start_maintenance: %s / end_maintenance: %s) [failed]" % (cluster, start_date, end_date))
                        state = 400
                else:
                    jmsg['message'] = "Maintenance DB not found"
                    jmsg['status'] = 'err'
                    self.logger.doLog("served '/maintenance/%s' [err: maintenance DB does not exist]" % (cluster))
                    state = 400
                return (jsonify(jmsg), state)
        ...
        def delete_programmed_maintenance(cluster):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/maintenance/remove/<string:cluster>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            ...
            if ((self.db_active3 == True) and (validFlag == True)):
                ret = self.idb_c3.query('DELETE FROM "systemMaintenance" WHERE time = %d' % (timestamp))
                jmsg['message'] = "maintenance date removed for cluster '%s'" % (str(cluster))
                self.logger.doLog("served '/maintenance/remove/%s' [ok]" % (cluster))
                jmsg['status'] = 'ok'
            else:
                if (validFlag == True):
                    jmsg['message'] = "cluster '%s' is not present in the allocator list" % (str(cluster))         
                    jmsg['status'] = 'war'
                    self.logger.doLog("served '/maintenance/remove/%s' [war: cluster is not valid]" % (cluster))
                else:
                    jmsg['message'] = "the database is not available"         
                    jmsg['status'] = 'err'
                    self.logger.doLog("served '/maintenance/remove/%s' [err: database is not available]" % (cluster))    
            return (jsonify(jmsg), state)
        ...
        def request_site_benchmark():
            ...
            if token['access_token'] == None or token['refresh_token'] == None:
                jmsg['message'] = "error when exchanging tokens"
                jmsg['status'] = 'err'
                self.logger.doLog("served '/evaluate/machines' [err: error when exchanging tokens ]")
                state = 500
                return (jsonify(jmsg), state)
            ...
            if request.is_json == False:
                ...
                jmsg['status'] = 'err'
                self.logger.doLog("served '/evaluate/machines' [err: wrong body type (not json)]")
                state = 400
                return (jsonify(jmsg), state)
            request.get_json()
            ...
            if (self.db_active2 != True):
                ...
                jmsg['status'] = 'err'
                self.logger.doLog("served '/evaluate/machines' [err: database '%s' is not active or has been deleted]" % (self.db_name_2))
                state = 400
            return (jsonify(jmsg), state)
        ...
        def show_site_rank(job_id):
            ...
            auth_res = self.auth(args['Authorization'])
            if auth_res['status'] != 200 or auth_res['jmsg'] == "not active":
                self.logger.doLog("served '/job/rank/<int:job_id>' [auth err: -- status (%d), msg (%s)]" % (auth_res['status'], auth_res['jmsg']))
                return (jsonify(auth_res['jmsg']), auth_res['status'])
            if (self.db_active2 != True):
            ...
            points = list(rs2.get_points(measurement='machineEvaluation', tags={'job_id':str(job_id), 'job_type':args['type']}))
            barrier = args['no_points']
            if (barrier > len(points)):
                barrier = len(points)
            for i in range(0, barrier):
                jpt[i] = "time (%s): job_id (%s) with type '%s' got evaluation <%s>\n" % (points[i]['time'], points[i]['job_id'], points[i]['job_type'], points[i]['rank'])
                i += 1
            jmsg['rank'] = jpt
        ...
        @self.service.route('/evaluate/machines/remove/<int:job_id>', methods=['DELETE'])        
        def remove_job_benchmark(job_id):
            ...
            if not self.platform.job_id_exists(job_id):
                ...
                jmsg['status'] = 'ok'
                self.logger.doLog("served '/evaluate/machines/remove/%d [ok]'" % (job_id))
            return (jsonify(jmsg), state)
