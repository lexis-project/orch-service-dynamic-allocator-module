        ...
        if (validFlag == False):
            ...
            # client 3
                ...
                if (self.db_active3):
                    ...
                    # entries
                    ret = cli.run(
                        ['./get_maintenance.sh', cluster], stdout=False)
                    self.logger.doLog(
    ...
    def get_speed_perf(self, src, dst, size):
        ...
        if ((center_exist_s) and (center_exist_d)):
            ...
            if (self.db_active1):
                ...
                # eventually remove the temporary file
                ret = cli.run(
                    ['rm', '../dbs/lxm_speed_perf.txt'], stdout=False)
                jmsg = {'message': jptv}
                jmsg['status'] = 'ok'
                state = 200
                if (float(size) <= 0):
            ...
            for item in ret[0]['message']:
                if size != item['size']:
                    ...
                    delete_ret = self.idb_c1.query(
                        ...
                        (timestamp))
            if (found):
                jmsg['message'] = "netperf item removed for dest center '%s', source center '%s' and size '%s'" % (
                    str(dst), str(src), str(size))
            else:
                jmsg['message'] = "no previous netperf items for dest center '%s', source center '%s' and size '%s'" % (
                    str(dst), str(src), str(size))
        ...
        if (self.db_active2):
            for queue in job_list['val']:
                ...
                if job_list['params']['type'] == "cloud" or (
                        job_list['params']['type'] == "both" and "cluster_id" not in queue['dest'].keys()):
                    m = str(queue['dest']['location']) + "_cloud"
                    jmm[m] = queue['mean']
                else:
                    m = str(queue['dest']['location']) + "_" + \
    ...
    def manageRoutes(self):
        ...
        def store_netperf(measure):
            ...
            if ((center_exist_s) and (center_exist_d) and (self.db_active1)):
                ...
                if (self.db_active1 == False):
                    ...
                    jmsg['message'] = "source cluster '%s' and/or destination cluster '%s' do not exist" % (
                        args['src'], args['dst'])
                    jmsg['status'] = 'err'
                    self.logger.doLog(
        ...
        def dumpdb(database):
            ...
            if (database == self.db_name_1):
                ...
                ret = cli.run(['influxd', 'backup', '-portable',
                              '-database', self.db_name_1, target])
                if (ret.returncode == 0):
                ...
                ret = cli.run(['influxd', 'backup', '-portable',
                              '-database', self.db_name_2, target])
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly dumped" % (
                        database)
                    jmsg['status'] = "ok"
                ...
                ret = cli.run(['influxd', 'backup', '-portable',
                              '-database', self.db_name_3, target])
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly dumped" % (
                        database)
                    jmsg['status'] = "ok"
        ...
        def restoredb(database):
            ...
            if (database == self.db_name_1):
                target = db_dump_path + '/_' + self.db_name_1 + '_dump'
                self.idb_c1.drop_database(self.db_name_1)
                ret = cli.run(['influxd', 'restore', '-db',
                              self.db_name_1, '-portable', target])
                if (ret.returncode == 0):
                ...
                self.idb_c2.drop_database(self.db_name_2)
                ret = cli.run(['influxd', 'restore', '-db',
                              self.db_name_2, '-portable', target])
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly restored" % (
                        database)
                    jmsg['status'] = "ok"
                ...
                self.idb_c3.drop_database(self.db_name_3)
                ret = cli.run(['influxd', 'restore', '-db',
                              self.db_name_3, '-portable', target])
                if (ret.returncode == 0):
                    jmsg['message'] = "database %s has been correctly restored" % (
                        database)
                    jmsg['status'] = "ok"
                ...
                if (self.db_active3):
                    db_body = [{'measurement': 'systemMaintenance',
                                'tags': {'cluster': cluster,
                                         'id': self.idx3},
                                'fields': {'start_maintenance': start_date,
        ...
        def delete_programmed_maintenance(cluster):
            ...
            if ((self.db_active3) and (validFlag)):
                ...
                if (validFlag):
                    jmsg['message'] = "cluster '%s' is not present in the allocator list" % (
                        str(cluster))
                    jmsg['status'] = 'war'
                    self.logger.doLog(
        ...
        def request_site_benchmark():
            ...
            if (self.db_active2 != True):
                jmsg['message'] = "the database ('%s') is not active or has been deleted" % (
                    self.db_name_2)
                jmsg['status'] = 'err'
                self.logger.doLog(
        ...
        def request_site_list(job_id):
            ...
            if not self.platform.job_id_exists(job_id):
                ...
                if self.platform.get_job_info(job_id)['status'] != "done":
                    jmsg['message'] = []
                    jmsg['status'] = self.platform.get_job_info(
                        job_id)['status'] + ". " + self.platform.get_job_info(job_id)['msg']
                else:
        ...
        @self.service.route('/evaluate/machines/remove/<int:job_id>',
                            methods=['DELETE'])
        def remove_job_benchmark(job_id):
            ...
            if not self.platform.job_id_exists(job_id):
                ...
                jmsg['status'] = 'ok'
                self.logger.doLog(
                    "served '/evaluate/machines/remove/%d [ok]'" %
                    (job_id))
            return (jsonify(jmsg), state)
