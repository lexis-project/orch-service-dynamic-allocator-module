...
class Clusters(object):
    ...
    def __init__(self, logger, lxc, api):
        self.logger = logger
        self.api = api
        self.job_list_file = '../dbs/' + \
            lxc.lxm_conf["lxm_db2"] + '_persistent_job_list'
        ...
        ids = range(list_len)
        for i in ids:
            self.default_transfer_speeds[self.transfer_sizes[i]
                                         ] = self.transfer_speeds[i]
        ...
        for idx, center in enumerate(self.center_list):
            ...
            if (self.heappe):
                ...
                if self.clusters_info[center]['hpc'].info_dict:
                    ...
                    self.logger.doLog(
                        "Could not connect to Heappe instance for %s" %
                        (center))
    ...
    def get_best_origin(self, sizeXfile, dest_zone,
                        origin_zones, default=False):
        origin_speed = 0
        origin = ""
        for zone in origin_zones:
            ...
            if zone_name == dest_zone:
                return 0, zone_name
            speed = self.get_av_speed(
                sizeXfile, dest_zone, zone_name, default=default)
            if speed == 0:
                return False, False
...
<<<<<<< HEAD
            ...
            if not origins:
                ...
                data_tranf_score = 1 - \
                    (total_max_time /
                     (total_max_time + job_args['max_walltime']))
            check = True
...
>>>>>>> origin/main
    ...
    def Cloud_weighted_criteria_mean(
        ...
        if not info_dict:
            self.logger.doLog(
                "Openstack auth failed/unreachable for center %s" %
                (center.name))
            return False
        check = True
<<<<<<< HEAD
...
>>>>>>> origin/main
        ...
        for flavour, flavour_features in info_dict['flavours'].items():
            if flavour_features['ram'] >= job_args['mem'] and flavour_features['vcpus'] >= job_args['vCPU']:
                selected_flavours.append({'flavour': flavour,
        ...
        for value in values:
            if value < 0:
                self.logger.doLog(
                    "Not enough Openstack resources to allocate task.")
                self.logger.doLog("score < 0. Values = ")
                self.logger.doLog(values)
                return True
            res['real_mean'] += value
    ...
    def backend_av_resources(self, projectID, token):
        ...
        if (r.status_code == 200):
            ret = (x for x in r.json()
                   if x["AssociatedLEXISProject"] == projectID)
            return ret
        else:
    ...
    def get_banned_sites(self, original_request_id):
        ...
        # evaluate all the available machines based on the weighted criteria
        # mean
    def evaluate(self, args, token):
        ...
        if args['type'] == "both" or args['type'] == "cloud":
            ...
            for item in items:
                for center in self.clusters_info:
                    if "cloud" not in self.clusters_info[center].keys() or (
                            center != item["HPCProvider"] and center != item["HPCProvider"].lower()) or center in banned_sites_cloud:
                        continue
                    else:
            ...
            for item in items:
                for center in self.clusters_info:
                    if "hpc" not in self.clusters_info[center].keys() or (
                            center != item["HPCProvider"] and center != item["HPCProvider"].lower()):
                        continue
                    else:
        ...
        with shelve.open(self.job_list_file, writeback=True) as job_list:
            if check:
                ...
                job_list[args['job_id']
                         ]['msg'] = "evaluation successfully ended."
            else:
                job_list[args['job_id']]['status'] = "err"
                job_list[args['job_id']]['msg'] = "evaluation failed."
            if self.api.write_evaluation(args['job_id'], dict(
                    job_list[args['job_id']])) == False:
                job_list[args['job_id']]['msg'] = job_list[args['job_id']]['msg'] + \
            ...
            for i in temp:
                if (len(results) >= job_list[job_id]['params']['number']):
                    ...
                    if 'cluster_id' in i['dest'].keys():
                        ...
                        cluster = "cloud"
                    job_list[job_id]['res'].append(
                        (i['dest']['location'], cluster))
        return (results)
