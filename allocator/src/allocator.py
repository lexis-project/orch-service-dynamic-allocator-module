...
def main():    
    try:
        ...
        lxc.idb_c1 = InfluxDBClient(host=lxc.lxm_conf["influx_server"], port=lxc.lxm_conf["influx_port"])
        lxc.idb_c2 = InfluxDBClient(host=lxc.lxm_conf["influx_server"], port=lxc.lxm_conf["influx_port"])
        lxc.idb_c3 = InfluxDBClient(host=lxc.lxm_conf["influx_server"], port=lxc.lxm_conf["influx_port"])
        ...
        if(lxc.lxm_conf["hard_startup"] == True): 
            logger.doLog("hard-startup mode is active")
            logger.doLog("database '%s' is dropped before starting application" % (lxc.lxm_conf["lxm_db1"]))
            lxc.idb_c1.drop_database(lxc.lxm_conf["lxm_db1"])
            logger.doLog("database '%s' is dropped before starting application" % (lxc.lxm_conf["lxm_db2"]))
            lxc.idb_c2.drop_database(lxc.lxm_conf["lxm_db2"])
            logger.doLog("database '%s' is dropped before starting application" % (lxc.lxm_conf["lxm_db3"]))
            lxc.idb_c3.drop_database(lxc.lxm_conf["lxm_db3"])
        # log the hard exit action (it will be ignored if application is running in background and it will be killed)
        if (lxc.lxm_conf["hard_exit"] == True):
            logger.doLog("hard-exit mode is active (the DB will be dropped when closing)")
        ...
        for entry in dbs:
            if (entry['name'] == lxc.lxm_conf["lxm_db1"]):
                logger.doLog("database ('%s') has already been created" % (lxc.lxm_conf["lxm_db1"])) 
                db_exist = True
                break
        ...
        for entry in dbs:
            if (entry['name'] == lxc.lxm_conf["lxm_db2"]):
                logger.doLog("database ('%s') has already been created" % (lxc.lxm_conf["lxm_db2"])) 
                db_exist = True
                break
            if (lxc.lxm_conf["debug"] == 1):
                print ("  [%-2d]: %s" % (i, str(entry['name'])))
        ...
        for entry in dbs:
            if (entry['name'] == lxc.lxm_conf["lxm_db3"]):
                logger.doLog("database ('%s') has already been created" % (lxc.lxm_conf["lxm_db3"])) 
                db_exist = True
                break
            if (lxc.lxm_conf["debug"] == 1):
                print ("  [%-2d]: %s" % (i, str(entry['name'])))
        ...
        logger.forceFlush()
        if (lxc.lxm_conf["hard_exit"] == True):
            logger.doLog("removing the streaming database (%s)" % (lxc.lxm_conf["lxm_db1"]))
            logger.doLog("removing the streaming database (%s)" % (lxc.lxm_conf["lxm_db2"]))
            logger.doLog("removing the streaming database (%s)" % (lxc.lxm_conf["lxm_db3"]))
            lxc.idb_c1.drop_database(lxc.lxm_conf["lxm_db1"])
            lxc.idb_c2.drop_database(lxc.lxm_conf["lxm_db2"])
            lxc.idb_c3.drop_database(lxc.lxm_conf["lxm_db3"])
        logger.closeLog()
