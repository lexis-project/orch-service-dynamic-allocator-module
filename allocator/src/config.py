...
class LXMconfig:
    ...
    def getConfiguration(self, conf_path):
        ...
        while (go == True):
            ...
            if (line == ""):
                ...
                elif (param == "heappe_service_URLs" and self.lxm_conf["heappe_middleware_available"] == 1):
    ...
    def postConfiguration(self):
        if (self.lxm_conf["debug"] == 1):
            ...
            print("  --------------------------------------------------------")
            if (self.lxm_conf["hard_exit"] == True):
                print("  hard-exit mode    : True")
            else:
                print("  hard-exit mode    : False")
            if (self.lxm_conf["hard_startup"] == True):
            ...
            print(
                "  service port      : %-5d " %
                (self.lxm_conf["service_port"]))
            print(
                "  influxDB address  : %s " %
                (self.lxm_conf["influx_server"]))
            print(
                "  influxDB port     : %-5d " %
                (self.lxm_conf["influx_port"]))
